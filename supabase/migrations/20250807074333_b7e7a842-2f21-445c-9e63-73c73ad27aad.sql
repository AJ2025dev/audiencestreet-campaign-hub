...
CREATE TABLE public.commissions (
  ...
  applies_to_user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE, -- For admin profit tracking which user it applies to
...
CREATE TABLE public.campaigns (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  agency_id UUID REFERENCES auth.users(id) ON DELETE SET NULL, -- If created by agency for advertiser
  name TEXT NOT NULL,
  description TEXT,
