-- Create a function to get daily performance metrics for agency charts
CREATE OR REPLACE FUNCTION public.get_agency_daily_performance_metrics(agency_id uuid, days_back integer DEFAULT 30)
RETURNS TABLE (
  date date,
  impressions bigint,
  clicks bigint,
...
AS $$
  ...
  agency_advertisers AS (
    SELECT advertiser_id
    FROM public.agency_advertisers
    WHERE agency_id = agency_advertisers.agency_id AND is_active = true
  ),
  daily_impressions AS (
    ...
    JOIN public.campaigns c ON c.user_id = aa.advertiser_id
    JOIN public.impression_tracking it ON it.campaign_id = c.id
  ...
  daily_clicks AS (
    ...
    FROM agency_advertisers aa
    JOIN public.campaigns c ON c.user_id = aa.advertiser_id
    JOIN public.clicks cl ON cl.campaign_id = c.id
